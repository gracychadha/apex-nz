<section class="rts-offer-area rts-section-gap">
    <div class="container">
        <div class="title-style-one center">
            <span class="pre radius-10 text-capitalize">What We Do</span>
            <h2 class="title-two rts-text-anime-style-1">What We Offer</h2>
            <p class="disc">We aim to simplify the way you track and manage your vehicles.<br> By understanding your
                requirements, we offer reliable GPS solutions<br> that enhance safety, lower expenses, and improve your
                daily operations.</p>
        </div>
        <div class="rts-offer-wrapper">
            <div class="tab-content">
                <div class="tab-pane fade show active" id="tab1">
                    <img src="{{ asset('website/images/offer/05.webp') }}" alt="">
                </div>

                <div class="tab-pane fade" id="tab2">
                    <img src="{{ asset('website/images/offer/02.png') }}" alt="">
                </div>

                <div class="tab-pane fade" id="tab3">
                    <img src="{{ asset('website/images/offer/03.jpg') }}" alt="">
                </div>

                <div class="tab-pane fade" id="tab4">
                    <img src="{{ asset('website/images/offer/04.png') }}" alt="">
                </div>

                <div class="tab-pane fade" id="tab5">
                    <img src="{{ asset('website/images/offer/01.webp') }}" alt="">
                </div>


            </div>
            <div class="tab-nav">
                <div class="nav nav-tabs" id="tab" role="tablist">
                    <button class="nav-link active" data-bs-toggle="tab" data-bs-target="#tab1">
                        <span class="main">Custom GPS Solutions</span>
                        <span class="second">Built Around Your Business.</span>
                    </button>

                    <button class="nav-link" data-bs-toggle="tab" data-bs-target="#tab2">
                        <span class="main">Fleet Management & Automation</span>
                        <span class="second">Control Operations. Reduce Costs.</span>
                    </button>

                    <button class="nav-link" data-bs-toggle="tab" data-bs-target="#tab3">
                        <span class="main">GPS Security & Theft Protection</span>
                        <span class="second">Protect What Matters.</span>
                    </button>

                    <button class="nav-link" data-bs-toggle="tab" data-bs-target="#tab4">
                        <span class="main">Data Analytics & Smart Reporting</span>
                        <span class="second">Analyze. Optimize. Grow.</span>
                    </button>

                    <button class="nav-link" data-bs-toggle="tab" data-bs-target="#tab5">

                        <span class="main">Real-Time GPS Vehicle Tracking</span>
                        <span class="second">Track Smarter. Move Faster.</span>
                    </button>


                </div>
            </div>
        </div>
    </div>
</section>